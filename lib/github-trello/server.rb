...
module GithubTrello
  class Server < Sinatra::Base
    post "/posthook" do
      ...
      puts "hello"
      if config[committer]
        then  if config[committer]["board_ids"][repo]
                  #we're golden, do everything! 
                  puts repo
              else puts "[Error] Repo not associated with a board_id. Run rake add_board"
                 end
      else
        #ask them to set up name
...
#     end
    ...
    def self.config=(config)
      @config = config
      @http = GithubTrello::HTTP.new(config["burricks"]["oauth_token"], config["burricks"]["api_key"])
    end
